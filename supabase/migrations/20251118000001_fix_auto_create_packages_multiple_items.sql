-- Fix auto_create_packages to create one package per transaction item, not one per transaction
--
-- BUG: When customer buys 2+ packages of same type in one transaction, only 1 package was created
-- ROOT CAUSE: Duplicate check looked for existing packages by (transaction_id + package_type_id)
--             After first item created a package, second item found it and skipped
--
-- FIX: Remove the duplicate check within the item loop
--      - Each transaction_item should create its own package
--      - Duplicate prevention is already handled by trigger (fires once per transaction)
--      - If someone buys 2x "Early Bird +", they should get 2 packages
...
BEGIN
    ...
    LOOP
        ...
        IF FOUND THEN
            ...
            -- REMOVED: Duplicate check that prevented creating multiple packages
            -- OLD CODE:
            ...
            -- LIMIT 1;
            --
            -- NEW BEHAVIOR: Create one package per transaction item
            -- If customer orders 2x "Early Bird +", they get 2 packages
...
Creates ONE package per transaction item - if customer buys 2x Early Bird+, they get 2 packages.
...
FIXED (2025-11-18): Removed duplicate check that prevented multiple packages of same type per transaction.
FIXED: Correctly casts varchar columns to text to match function signature.
