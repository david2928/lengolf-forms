-- Fix type mismatch in auto_create_packages_from_transaction
-- Issue: products.products.name is varchar(200) but function returns text
-- Error: "Returned type character varying(200) does not match expected type text in column 2"
-- This error only occurs when transactions have products mapped to package types (triggers auto-creation)
...
BEGIN
    ...
    FOR v_item IN
        SELECT
            ti.*,
            p.name::text as product_name,  -- FIXED: Cast varchar(200) to text
            c.id as customer_id,
            c.customer_name,
        ...
        IF FOUND THEN
            ...
            IF v_existing_package IS NOT NULL THEN
                ...
                RETURN QUERY SELECT
                    v_existing_package,
                    v_item.product_name,
                    v_package_type.name::text,  -- FIXED: Cast varchar to text
                    FALSE,
