...
import type { DailySLATrend, SLAAPIResponse } from '@/types/chat-sla';

/**
 * GET /api/chat-sla/daily-trends
 * Get daily SLA compliance trends for chart visualization
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  ...
  if (!session?.user?.email) {
    return NextResponse.json<SLAAPIResponse<DailySLATrend[]>>(
  ...
  try {
    ...
    const { data, error } = await refacSupabaseAdmin.rpc('get_chat_sla_daily_trends', {
    ...
    if (error) {
      console.error('Error fetching daily SLA trends:', error);
      throw error;
    }

    return NextResponse.json<SLAAPIResponse<DailySLATrend[]>>({
      success: true,
      data: (data || []) as DailySLATrend[]
    });

  } catch (error) {
    console.error('Error in chat SLA daily trends API:', error);
    return NextResponse.json<SLAAPIResponse<DailySLATrend[]>>(
      {
        success: false,
