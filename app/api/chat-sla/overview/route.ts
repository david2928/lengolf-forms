...
import type { ChatSLAOverview, SLAAPIResponse } from '@/types/chat-sla';

/**
 * GET /api/chat-sla/overview
 * Get overall SLA metrics for specified date range
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  ...
  if (!session?.user?.email) {
    return NextResponse.json<SLAAPIResponse<ChatSLAOverview>>(
  ...
  try {
    ...
    const { data, error } = await refacSupabaseAdmin.rpc('get_chat_sla_overview', {
    ...
    return NextResponse.json<SLAAPIResponse<ChatSLAOverview>>({
      success: true,
      data: data as ChatSLAOverview
    });

  } catch (error) {
    console.error('Error in chat SLA overview API:', error);
    return NextResponse.json<SLAAPIResponse<ChatSLAOverview>>(
      {
        success: false,
