...
export async function POST(request: NextRequest) {
  console.log('📄 Print Bill Bluetooth API: Request received');
  ...
  try {
    ...
    console.log('📄 Print Bill Bluetooth: Fetching bill data for table session', tableSessionId);
    ...
    try {
      ...
      console.error('❌ Print Bill Bluetooth: Error fetching bill data:', error);
    ...
    console.log('✅ Print Bill Bluetooth: Bill data prepared successfully', {
    ...
    // The client-side service will handle the actual Bluetooth printing
    return NextResponse.json({
      success: true,
      billData: receiptData,
      escposData: Array.from(escposData), // Convert Uint8Array to regular array for JSON
      message: `Bill for table ${billData.tableNumber} ready for Bluetooth printing`,
      method: 'Web Bluetooth API',
      summary: BillDataService.getBillSummary(billData)
    });

  } catch (error) {
    console.error('❌ Print Bill Bluetooth API: Unexpected error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
