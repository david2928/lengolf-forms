...
const WeeklyReportsTable: React.FC = () => {
  const [data, setData] = useState<WeeklyReportRow[]>([]);
  ...
  const fetchWeeklyReports = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/sales/weekly-reports', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch weekly reports: ${response.statusText}`);
      }

      const result: WeeklyReportsResponse = await response.json();
      setData(result.data || []);
    } catch (err) {
      console.error('Weekly reports fetch error:', err);
  ...
  const handleExportCSV = () => {
    ...
    link.download = `weekly-sales-report-${currentYear}.csv`;
    link.click();
  };
  ...
  if (error) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-red-600">Error Loading Weekly Reports</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-gray-600 mb-4">{error}</p>
          <Button onClick={fetchWeeklyReports} variant="outline">
  ...
  // Limit to last 9 weeks for better display
  const displayData = data.slice(-9);
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-3 text-gray-600">Loading weekly reports...</span>
          </div>
        ) : data.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            <BarChart3 className="h-12 w-12 mx-auto mb-4 text-gray-300" />
            <p>No weekly data available for {currentYear}</p>
          </div>
        ) : (
              ...
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {displayData.map((week) => (
                    <th key={week.week} className="px-3 py-3 text-center text-xs font-medium text-gray-500">
                      <div 
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {metrics.map((metric, index) => (
                  <tr key={metric.key} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {displayData.map((week) => (
                      <td key={`${week.week}-${metric.key}`} className="px-3 py-4 text-sm text-center text-gray-900">
                        {metric.format(week[metric.key as keyof WeeklyReportRow] as number)}
                      </td>
                    ))}
                  </tr>
                ))}
