...
export function EnhancedBookingTypeSelector({
  ...
  const handleTypeSelect = (typeValue: string) => {
    ...
    if (typeValue === 'Coaching' && (value === 'Coaching' || value?.startsWith('Coaching ('))) {
      onChange('')  // Clear coaching selection entirely
    } else if (value === typeValue) {
      onChange('')  // Clear selection for other types
    } else {
      onChange(typeValue)
    }
  }
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {primaryBookingTypes.map((type) => {
            ...
            const isSelected = type.value === 'Coaching' 
              ? (value === 'Coaching' || value?.startsWith('Coaching ('))
              : value === type.value
            const isUnselected = value && !isSelected
